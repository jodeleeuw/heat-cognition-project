var jsPsychSurveyVertSlider = (function (jspsych) {
  'use strict';

  var version = "2.0.0";

  const info = {
    name: "survey-vert-slider",
    version,
    parameters: {
  ...
  class SurveySliderPlugin {
    ...
    trial(display_element, trial) {
      ...
      var html = '<div id="jspsych-html-slider-response-wrapper" style="max-width: 100vw; width: 100%; box-sizing: border-box; overflow-x: hidden; display: flex; justify-content: center;">';
      html += '<div class="jspsych-html-slider-response-container" style="position:relative; width: 100%; max-width: 100%; box-sizing: border-box; padding: 0 20px; ';
      if (trial.slider_width !== null) {
        html += "max-width: " + trial.slider_width + "px;";
      }
      html += '">';
      if (trial.preamble !== null) {
        html += '<div style="margin-bottom: 30px; text-align: center; max-width: 100%; word-wrap: break-word;" id="jspsych-survey-slider-preamble" class="jspsych-survey-slider-preamble">' + trial.preamble + "</div>";
      }
      html += '<form id="jspsych-survey-slider-form" style="max-width: 100%; overflow-x: hidden;">';
      
      html += '<div style="display: flex; flex-direction: row; justify-content: center; align-items: flex-start; gap: 30px; flex-wrap: wrap; max-width: 100%; box-sizing: border-box;">';
      var question_order = [];
      for (var i = 0; i < trial.questions.length; i++) {
      ...
      for (var i = 0; i < trial.questions.length; i++) {
        ...
        html += '<div style="display: flex; flex-direction: column; align-items: center; text-align: center; min-width: 120px; flex: 0 0 auto;">';
        
        if (question.stimulus) {
          html += '<div id="jspsych-html-slider-response-stimulus-' + i + '">' + question.stimulus + "</div>";
        }
        
        html += '<div style="height: 300px; width: 120px; display: flex; align-items: center; justify-content: center;">';
        html += '<input style="width: 300px; transform: rotate(-90deg); transform-origin: center center; margin: 0;" type="range" class="jspsych-slider" value="' + question.slider_start + '" min="' + question.min + '" max="' + question.max + '" step="' + question.step + '" id="jspsych-html-slider-response-response-' + i + '" name="Q' + i + '" data-name="' + question.name + '"></input>';
        html += '</div>';
        
        html += '<label class="jspsych-survey-slider-statement" style="margin-top: 10px; font-weight: bold;">' + question.prompt + "</label>";
        
        if (question.ticks && question.ticks.length > 0) {
          html += '<div style="margin-top: 5px;">';
          for (var j = 0; j < question.ticks.length; j++) {
            html += '<div style="font-size: 80%;">' + question.ticks[j] + "</div>";
          }
          html += '</div>';
        }
